        ...
        case 'makefile.pl':
            ...
            .then(function getAllMeta(possibleCpanOut) {
                ...
                if (metainfo.length === 0) {
                    console.error('No meta.json files or cpan output were retrieved after running perl '+ file)
                } else {
                    return metainfo
                }
            })
...
function extractStdOutFormat(stdoutFile) {
    return fs.readFileAsync(stdoutFile, "utf8")
    .then(function(rawContents){
        /** 
        ...
        */
        ...
        source_item = {
            'id': null,
            'version': null,
            'filePath': path.dirname(stdoutFile)
        }
...
function execCpanmShowdeps(file, dir){
    var show_deps_file = 'tempDepsList.txt'
    return new Promise( function(resolve, reject) {
        child_process.exec('cpanm --showdeps -q ' + dir + ' > ' + show_deps_file, function(err, stdout, stderr) {
            if (err || stderr) {
                ...
                return resolve(show_deps_file)
            }
        })
    }).then(function(stdoutFile){
        return extractStdOutFormat(show_deps_file)
    }).catch( function(err) {
        console.error('Error in parsing output of cpanm --showdeps ' + err) // TODO: should entire process fail if this fails
    })
}

function execPrereqScanner(dir){
    var perl_output_file = 'scan_perl_prereqs_output.txt'
    return new Promise( function(resolve, reject){
        child_process.exec('scan-perl-prereqs > ' + perl_output_file, function(err, stdout, stderr) {
            if (err || stderr) {
                console.error(err || stderr )
        ...
        return extractStdOutFormat(perl_output_file)
    }).catch(function (err) {
        console.error('Error in parsing output of scan-perl-prereqs ' + err) // TODO: should entire process fail if this fails
    })
}
...
function execMakefilepl(file, dir) {
    ...
    return new Promise( function(resolve, reject) {
        child_process.exec('cd ' + dir + '; perl ' + fileLoc + ' > tempmakeoutput.txt ; cd -', function(err, stdout, stderr) {
            if (err || stderr ) {
                var examinedErr = err || stderr
                // don't care about some errors. the desired effect should still happen.
                if (examinedErr.includes('Cannot determine perl version') || 
                    ...
                    // Maybe we should remove this check and resolve everything
...
function execBuildpl(file, dir){
    fileLoc = path.join(dir, file)
    return new Promise(function (resolve, reject) {
        child_process.exec('cd ' + dir + '; perl ' + fileLoc + ' > tempbuildoutput.txt ; cd -', function (err, stdout, stderr) {
            if (err || stderr) {
...
module.exports = {
    ...
    execPrereqScanner: execPrereqScanner,
    extractStdOutFormat: extractStdOutFormat,
    findFiles: findFiles
}