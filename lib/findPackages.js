...
function determine(dir, ignores) {
    ...
    .then(function(origMetaInfo){
        ...
        .then(function(filesFound) {
            ...
            return Promise.all(promises).then(function(allResolved) {
                var concatted = allResolved.concat(origMetaInfo)
                if (concatted.length <= 1){
                    return concatted
                } else {
                    return concatted.reduce(function (a, b) {
                        return a.concat(b)
                    })
                }
            })
    ...
    .then(function catchAllPreReqsScan(metas){
        return utils.execPrereqScanner(dir)
        .then(function mergeMetas(scanPerlPreReqOut){
            metas.concat(scanPerlPreReqOut)
            return metas
        })
    ...
    }).then(function injectFiles(metaInfos){
        return utils.findFiles(dir).then( function(filesFound) { 
            return metaInfos.map(function(meta){
                meta['files'] = filesFound
                return meta
            })
        })
    })
